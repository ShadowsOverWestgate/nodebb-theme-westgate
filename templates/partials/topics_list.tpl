			...
			<div class="flex-grow-1 d-flex flex-wrap gap-1 position-relative">
				...
				<span component="topic/labels" class="d-flex flex-wrap gap-1 w-100">
					...
					<span data-tid="{./tid}" component="topic/tags" class="lh-1 tag-list d-flex flex-wrap gap-1 {{{ if !./tags.length }}}hidden{{{ end }}}">
						{{{ each ./tags }}}
						<a href="{config.relative_path}/tags/{./valueEncoded}"><span class="badge border border-gray-300 fw-normal tag tag-class-{./class}" data-tag="{./value}"><i class="fa fa-tag" aria-hidden="true"></i>{./valueEscaped}</span></a>
						{{{ end }}}
					</span>
